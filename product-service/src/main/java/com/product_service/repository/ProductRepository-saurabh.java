...
//    Product getProductById(@Param("productId") UUID productId);
    ...
    @Query(value = "select p.* from product p where p.archive=:archive",nativeQuery = true)
    Page<ProductEntity> getAllProduct(@Param("archive") boolean archive, Pageable pageable);
